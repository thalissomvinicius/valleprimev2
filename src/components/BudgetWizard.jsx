...
const BudgetWizard = ({ lot, onClose, obraName }) => {
    ...
    // Calculations
    const downPaymentTotal = calculateDownPayment(lotValue, formData.downPaymentPercent);
    const sinalDiscountedTotal = calculateDiscountedSinal(downPaymentTotal, formData.sinalDiscountEnabled, formData.sinalDiscountValue);
    const entradaAmount = formData.entradaEnabled ? parseFloat(formData.entradaValue) || 0 : 0;

    const { openBalance, totalWithDiscount } = calculateDiscountedValues(lotValue, downPaymentTotal, entradaAmount, formData.discountPercent);
...
// Step Components have been extracted to BudgetWizardSteps.jsx
