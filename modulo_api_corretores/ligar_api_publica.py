...
def update_frontend_api(url):
    ...
    try:
        ...
        # Padrao: const LOCAL_UAU_API = 'https://...';
        content = re.sub(r"const LOCAL_UAU_API = 'https://[^']+';", f"const LOCAL_UAU_API = '{url}';", content)
        
        # Retrocompatibilidade com padrões antigos se necessário
...
def run_cloudflare_tunnel():
    ...
    # Redireciona porta 8000 (onde a nova API roda)
    process = subprocess.Popen(
        [cf_path, "tunnel", "--metrics", "127.0.0.1:4567", "--url", "http://127.0.0.1:8000"],
    ...
    if public_url:
        ...
        print(f"  VENDAS:       {public_url}/api/vendas/28/70100")
        print(f"  MAPA:         {public_url}/api/disponibilidades/29/625")
...
def main():
    ...
    # Inicia o servidor uvicorn (FastAPI) na porta 8000
